...
const SendTokenSelector = ({ quickSelectTokens, onTokenSelect, selectedToken, hasSelectedToken }) => {
        ...
        <Dialog>
                    ...
                    <div className="flex items-center">
                        {hasSelectedToken ? (
                            <>
                                ...
                                <span className='mr-1 text-base font-medium text-muted-foreground transition opacity-100'>
                                    {selectedToken.symbol}
                                </span>
                            </>
                        ) : (
                            <span className='mr-1 text-base font-medium text-foreground transition opacity-100 ml-3'>
                                Select a Token
                            </span>
                        )}
                        <ChevronDown className={`w-4 h-4 ${hasSelectedToken ? 'text-muted-foreground' : 'text-foreground'}`} />
                    </div>
                </Button>
            </DialogTrigger>
            <DialogContent className="fixed left-[50%] top-[50%] z-50 grid w-full translate-x-[-50%] translate-y-[-50%] gap-4 border shadow-lg duration-200 data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:fade-out-0 data-[state=open]:fade-in-0 data-[state=closed]:zoom-out-95 data-[state=open]:zoom-in-95 sm:rounded-lg rounded-lg sm:max-w-[26rem] sm:top-[50%] bg-backgroundDark p-5 sm:pt-[1.1rem] pt-[2.5rem] max-w-[90vw] border-border-bright border-opacity-30 backdrop-blur-sm">
                <div className="flex items-center justify-between text-primary-foreground">
                    <DialogTitle className="tracking-tight text-[1.1rem] font-medium">Select a token</DialogTitle>
                    <DialogClose asChild>
                ...
                <div className="relative">
                    ...
                    <Input
                        ...
                        className="w-full h-12 rounded-xl pl-12 pr-12 bg-secondary/30 text-foreground border border-border-bright placeholder:text-muted-foreground focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-primary transition-colors"
                    />
                    {searchValue && (
                        <button
